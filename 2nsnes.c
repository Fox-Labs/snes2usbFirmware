...
static unsigned char snesReorderButtons(unsigned char bytes[2])
{
	 ...
	 * one single byte. */
    v =	    (bytes[0]&0x80)>>7; //Clock 1 = B //USB = 1
    v |=	(bytes[1]&0x01)<<1; //Clock 9 = A //USB = 2
    v |=	(bytes[0]&0x40)>>4; //Clock 2 = Y //USB = 3
    v |=	(bytes[1]&0x02)<<2; //Clock 10 = X //USB = 4

    v |=	(bytes[1]&0x04)<<2; //Clock 11 = L //USB = 5
    v |=	(bytes[1]&0x08)<<2; //Clock 12 = R //USB = 6
    v |=	(bytes[0]&0x20)<<1; //Clock 3 = Select //USB = 7
    v |=	(bytes[0]&0x10)<<3; //Clock 4 = Start //USB = 8    
...
const char two_nsnes_usbHidReportDescriptor[] PROGMEM = {
	/* Controller and report_id 1 */
    0x05, 0x01,			// USAGE_PAGE (Generic Desktop)
    0x09, 0x04,			// USAGE (Joystick)
    0xa1, 0x01,			//	COLLECTION (Application)
...
static void setupDescriptors()
{
	if (fourscore_mode || multitap_mode) {
		SnesGamepad.num_reports = 1;
		SnesGamepad.reportDescriptorSize = sizeof(fournsnes_usbHidReportDescriptor);
